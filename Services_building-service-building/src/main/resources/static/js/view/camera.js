 ...
 ******************************************/
$(function () {
    parent.cameras = []; //摄像机数组
    var earth = parent.earth; //全局earth
    var divHeight = $(parent.document).height() - 380;
    parent.cameraLayer = earth.Factory.CreateEditLayer(earth.Factory.CreateGuid(), "my_layer", earth.Factory.CreateLonLatRect(-90, 90, -180, 180, 0, 10), 0, 10, '');
    earth.AttachObject(parent.cameraLayer);
    ...
    $("#btnNew").click(function () {
        earth.Event.OnCreateGeometry = function (pt, type) {
            if (pt) {
                ...
                parent.cameraLayer.BeginUpdate();
                parent.cameraLayer.AttachObject(camera);
                parent.cameraLayer.EndUpdate();

                parent.cameras[guid] = camera;
    ...
    function createCamera(record) {
        ...
        parent.cameras[record["ID"]] = camera;
    }
            ...
            if (confirm("是否确定要删除？")) {
                ...
                for (var i = 0; i < camera.length; i++) {
                    ...
                    if (id === userId) {
                        ...
                        if (parent.cameras.hasOwnProperty(id)) {
                            earth.ToolManager.SphericalObjectEditTool.Browse();
                            var thisCamera = parent.cameras[id];
                            if (thisCamera) {
                                if (thisCamera.EnableMediaStream) {
                                    thisCamera.EnableMediaStream = false;
                                }
                            }
                            parent.cameraLayer.DetachWithDeleteObject(thisCamera);
                            parent.cameras[id] = "";
                        }
                        //保存文档
        ...
        if (userId) {
            ...
            for (var i = 0; i < camera.length; i++) {
                ...
                if (id === userId) {
                    ...
                    //TODO:由于目前没有获取摄像机模型目标点的接口，所以这里暂时注释掉
                    ...
                    var camera = parent.cameras[userId];
                    camera.RtspURL = obj.rtsp;
                    camera.MaskURL = obj.maskUrl;
                    ...
                    if (obj.cx && !isNaN(obj.cx) &&
                        obj.cy && !isNaN(obj.cy) &&
                        obj.cz && !isNaN(obj.cz)) {
                        camera.SphericalTransform.SetLocationEx(obj.cx, obj.cy, obj.cz);

                        if (obj.tx && !isNaN(obj.tx) &&
                            obj.ty && !isNaN(obj.ty) &&
                            obj.tz && !isNaN(obj.tz)) {
            ...
            callback: {
                ...
                onClick: function (event, treeId, node) {
                    if (node && node.id) {
                        ...
                        var camera = parent.cameras[node.id];
                        if (camera) {
                            //控制摄像机窗口最前
                            closeCameraWindows();
    ...
    function exportCameras(obj) {
        ...
        for (var i = 1; i < nodes.length; i++) {
            var camera = parent.cameras[nodes[i].id];
    ...
    function getSelectedCamera() {
        ...
        var camera = null;
        if (selNode) {
            camera = parent.cameras[selNode.id];
        }
        return camera;
    }
    ...
    function closeCameraWindows() {
        for (var i in parent.cameras) {
            if (parent.cameras[i]) {
                parent.cameras[i].EnableThumbWindow = false;
            }
        }
    }
    ...
    function close() {
        if (earth.UserDocument) {
            for (var id in parent.cameras) {
                if (parent.cameras.hasOwnProperty(id)) {
                    if (parent.cameras[id] != "") {
                        parent.cameraLayer.DetachWithDeleteObject(parent.cameras[id]);
                    }
                }
            }
            earth.DetachObject(parent.cameraLayer);
            parent.cameras = [];
            parent.cameraLayer = null;
        }
    }
