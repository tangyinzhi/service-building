...
$(function () {
    ...
    $("#paramSetInner").on("click", ".normalBtn", function (e) {
        ...
        switch (thisId) {
            ...
            case "btnSetting": //点击链路设置事件
                ...
                break;
            case "btnImport":
                earth.paramIp = parent.params.ip;
    ...
    $("#btnConfirm").click(function () {
        ...
        if (cameraRotationAngle >= parent.params.camearaMaxRotation && horizonRotate > 0) {
        ...
        if (currentCameraTilt >= parent.params.camearaMaxTilt && verRotate > 0) {
        ...
        if (cameraRotationAngle >= parent.params.camearaMaxRotation) {//旋转至最右侧(350度)，停止旋转
            horizonRotate = parent.params.camearaMaxRotation - cameraDictionary[curSelNodeId].cameraRotationAngle;
            cameraDictionary[curSelNodeId].cameraRotationAngle = parent.params.camearaMaxRotation;
        ...
        if (currentCameraTilt >= parent.params.camearaMaxTilt) {//旋转至最上方侧(90度)，停止旋转
            verRotate = parent.params.camearaMaxTilt - cameraDictionary[curSelNodeId].cameraTiltAngle;
            cameraDictionary[curSelNodeId].cameraTiltAngle = parent.params.camearaMaxTilt;
    ...
    var events = {
        "keyup": function (event) {
            ...
            if (id == "horizonRotate") { //左右旋转
                checkNum(thisObj, false, 2, parent.params.camearaMaxRotation);
            } else if (id == "verRotate") {
                checkNum(thisObj, false, 2, parent.params.camearaMaxTilt);
            } else if (id == "leftRightSpeed") {
                checkNum(thisObj, true, 2, parent.params.camearaMaxRotation);
            } else if (id == "updownSpeed") {
                checkNum(thisObj, true, 2, parent.params.camearaMaxTilt);
            }
...
function initDeviceList(callFuc) {
    ...
    gb28181.Gb28181Server = parent.params.ip;
...
function getCameraList(cameraListXml) {
    ...
    if (0 < resultnum) {
        ...
        for (i = 0; i < resultnum; i++) {
            ...
            //调用接口获取数据库相关字段
            earth.Gb28181CameraManager.Gb28181Server = parent.params.ip;
            ...
            if (!parent.params.isSimulationCamera) { //不启用模拟摄像头则开启过滤
...
function createCamera(lon, lat, deviceID, num, tilt, heading, rotation) {
    ...
    var altitude = earth.Measure.MeasureTerrainAltitude(lon, lat) + parent.params.cameraDefaultHeight; //默认离地高度10米
    elementCone.SphericalTransform.SetLocationEx(lon, lat, altitude);
    elementCone.SphericalTransform.SetRotationEx(90, parent.params.cameraDefaultHeading, 0);
    ...
    camera.SetGB28181ServerUrl(parent.params.ip);
...
function resetEarthToolHeight() {
    ...
    if (parent.earthToolsBalloon && oLength > 0) { //工具条调整
        parent.hideProfile();
        if (oLength <= 4) { //减少1/4
            parent.resizeEarthTool(parent.earthToolHeight * 3 / 4);
        } else { //减少1/2
            parent.resizeEarthTool(parent.earthToolHeight / 2);
        }
    } else { //还原
        parent.resizeEarthTool();
    }
