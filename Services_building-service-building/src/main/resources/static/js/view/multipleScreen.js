...
$(function () {
    var bMultiple = parent.bMultiple; // 当前是否是多屏显示状态
    ...
    $("#rdoLeft").click(function () {
        ...
        earthTag = 0;
        createLayerTree("#layerTree", parent.LayerManagement.earthArray[0]);
    });
    $("#rdoRight").click(function () {
        $("#rdoLeft").removeAttr("checked");
        $("#rdoRight").attr("checked", "checked");
        earthTag = 1;
        createLayerTree("#layerTree", parent.LayerManagement.earthArray[1]);
    });
    $("#btnCompare").click(function () {
        if (bSync || parent.bSync) {
            bSync = false; //首先关闭联动
            parent.bSync = bSync;
        ...
        if (bMultiple) { //当前多屏时
            if (type == 2) {
                parent.LayerManagement.showHistorySlider(false, true, true);
            }
            $(this).text("双屏显示");
            parent.isShowHistory = false;
            parent.setScreen(1, ""); // 恢复到一屏
            bMultiple = false;
            parent.bMultiple = bMultiple;
            ...
            createLayerTree("#layerTree", parent.LayerManagement.earthArray[0]); // 恢复为默认球的图层树
            if (type == 2) {
                if (!($(parent.document).find("#historyData").hasClass("selected"))) {
                    parent.LayerManagement.showHistorySlider(false);
                }
            }
        } else { //当前单屏时
            parent.isCloseSlider = true;
            $(this).text("单屏显示");
            bMultiple = true;
            parent.bMultiple = bMultiple;
            ...
            parent.setScreen(2, selectTag, false, function () {
                if (type == 2) {
                    parent.isShowHistory = true;
                    var flag = parent.BalloonHtml.getItemStyle("ViewTranSetting");
                    if (!flag) {
                        parent.BalloonHtml.removeItemStle("ViewTranSetting");
                        parent.setSlidersVisible(0);
                        earth.Event.OnGUISliderChanged = function () {
                        };
                        setTimeout(function () {
                            // var flag2 = parent.Tools.toolBarItemClickStyle("historyData");
                            parent.LayerManagement.showHistorySlider(true);
                        }, 300);
                    } else {
                        // var flag2 = parent.Tools.toolBarItemClickStyle("historyData");
                        parent.LayerManagement.showHistorySlider(true);
                ...
                parent.gotoPose(0)(); // 将其他屏定位到第一屏的位置
            });
...
function setAlphaSync(isMulti) {
    parent.isMultiScreen_2 = isMulti;
    if (isMulti) {
        if (parent.LayerManagement.earthArray[1]) {
            parent.LayerManagement.earthArray[1].Environment.TerrainTransparency = parent.LayerManagement.earthArray[0].Environment.TerrainTransparency;
        }
    }
}
