...
DEFFLD = {
    ...
    form_but: {
        html: ' <li class="pc-submit"><input id="btnSubmit" class="btn-submit" onclick="return false;" value="提交" type="button"></li>'
    },
    value_but: {
        html: ' <li class="pc-submit"><input id="saveValue" class="btn-submit" value="保存" type="button" hidden></li>'
    },
    text: {
    ...
    image: {
        html: '<label class="desc"><span class="req hide"> *</span></label><div class="content"><button type="button" class="layui-btn layui-btn-sm layui-btn-normal" id="info_upload">上传图片</button><div class="layui-upload"><div class="layui-upload-list info_all"></div></div></div>'
    },
    section: {
    ...
    date_ymd: '<label class="desc"><span class="req hide"> *</span></label><div class="content textcontent"><input type="text"  maxlength="255"  class="input fld data_ymd"  /></div>',
    date_mdy: '<label class="desc"><span class="req hide"> *</span></label><div class="content textcontent"><input type="text"  maxlength="255"  class="input fld data_ymd" /></div>',
    date_dmy: '<label class="desc"><span class="req hide"> *</span></label><div class="content textcontent"><input type="text"  maxlength="255"  class="input fld data_ymd" /></div>',
    time: {
    ...
    map: {
        html: '<label class="desc"><span class="req hide"> *</span></label><div class="content"><div class="map" id="mapDiv" ></div></div>'
    },
    address: {
        html: '<label class="desc"><span class="req hide"> *</span></label><div class="content textcontent"><input type="text"  maxlength="255" class="input fld" /></div>'
    ...
    address_en: '<textarea style="height:60px;"  class="input xxl detail" placeholder="Street"></textarea></span>',
    address_cn: '<textarea style="height:60px;"  class="input xxl detail" placeholder="详细地址"></textarea></span>',
    likert: {
...
function createFields() {
    ...
    if (M.GID != "") {
        ...
        a.append(DEFFLD['value_but'].html);
    }
...
function createField(r, q, num) {
    ...
    if (q.REQD === '1') {
        o.removeClass('hide');
        m.find(':radio,input,textarea,select').addClass('required');
    }
    p.text(q.LBL);
    p.append(o);
                                                ...
                                                if (q.TYP === 'likert') {
                                                    ...
                                                    if (q.TYP === 'dropdown2') {
                                                        ...
                                                        if (d !== '2') {
                                                            ...
                                                            for (var f = 0; f < d; f++) {
                                                                m.append('<select  class="input"></select>')
                                                            }
                                                            m.find('select').css({
                                                                width: (100 / d - 1) + '%',
                                                                'margin-right': '1%'
    ...
    if (q.CLMV) {
        m.find(':text,textarea,img,hidden').attr('id', q.CLMV);
        m.find('select').addClass(q.CLMV);

        if (q.TYP === 'radio' || q.TYP === 'checkbox') {
            if (q.TYP == 'radio') {
                radioLIst.push(q.CLMV);
            }
            if (q.TYP == 'checkbox') {
                boxLIst.push(q.CLMV);
            }
            m.attr('id', q.CLMV)
            m.find('input').attr('name', q.CLMV);
            // getEnum() ;

        }
        m.find(':text,textarea,select,img,hidden').attr('name', q.CLMV);
        if (q.TYP === 'image') {
            m.find('button').attr('name', q.CLMV);
        }
    }
...
//地图初始化 by wyl
function initMap() {
    var visual = getQueryString("visual");
    console.log(visual);
    $("li[typ='map']").attr('id', 'mapbox');
    if (visual == 'false') {
                                        ...
                                        if (n == 'name') {
                                            ...
                                            if (n == 'image') {
                                                ...
                                                if ($.isImage(f)) {
                                                    ...
                                                    r.find('.image-img').attr('src', '../images/form/picUp.png')
                                                }
                                            }
                                        }
                                    }
            ...
            if ($(this).attr('id') === 'btnSubmit') {
                ...
                $.postJSON(SAVEFORM,
                    ...
                    function (g) {
                        ...
                        if (g.msg) {
                            setTimeout(function () {
                                layer.msg('表单保存成功', {icon: 1, time: 200});
                                return
                            }, 700)
                        }
                    })
...
function initRule() {
    var c = function (e, d) {
        ...
        b = function (d) {
            $.each(RULE.FIELDSRULE || [], function (e, g) {
                if (d) {
                    var f = false;
    ...
    if (typeof (a) != 'undefined' && a.length < 50) {
        $('#form1').find('[typ=\'image\']').find('img').attr('src', '../images/form/picUp.png')
    }
}
...
function initImg() {
    if (!window.query) {
        $('#fields>li[typ!=\'goods\']').find('div.content div.goods-item img.img').on('click', function () {
            var a = $(this).attr('src');
            $.lightBox({
                url: '#stage',
                size: 'l cus3',
...
layui.use(['element', 'laydate'], function () {
    ...
    map = new ol.Map({
        ...
        view: new ol.View({
            center: [105.39814621210098, 28.89052122831345],
            zoom: 18,
            minZoom: 10,
            maxZoom: 20,
