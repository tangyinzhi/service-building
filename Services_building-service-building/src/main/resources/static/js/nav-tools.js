...
function keywordSearchClicked(id) {
    var selectLayer = LayerManagement.selectLayer;
    if (!selectLayer) {
        alert("请选择一个图层");
        return;
    } else {
...
function checkDpoiSearch(LayerList) {
    if (!LayerList) {
        LayerList = LayerManagement.earth.layerManager.LayerList;
    }
    if (LayerList.GetChildCount() > 0 && LayerList.Name != "xianji") {
        for (var i = 0; i < LayerList.GetChildCount(); i++) {
            var ChildLayer = LayerList.GetChildAt(i);
            if (ChildLayer == null) {
                continue;
            }
            if (ChildLayer.GetChildCount() > 0 && ChildLayer.Name != "xianji") {
                for (var i = 0; i < LayerList.GetChildCount(); i++) {
                    var thChildLayer = ChildLayer.GetChildAt(i);
                    if (thChildLayer == null) {
                        continue;
                    }
                    if (thChildLayer.Name == "xianji") {
                        LayerManagement.selectLayer = thChildLayer.Guid;
                        return;
                    }
                }
            }
...
//POI查询
function DpoiSearchClicked(id) {
    //var layerList = LayerManagement.earth.layerManager.LayerList;
    //checkDpoiSearch(layerList);
    LayerManagement.selectLayer = "2b906ff2-8d25-4785-9704-cf249629e8e3";//poi图层
    var selectLayer = LayerManagement.selectLayer;
    if (!selectLayer) {
        alert("请选择一个图层");
        return;
...
function modelYpClicked(id) {
    var flag = Tools.toolBarItemClickStyle(id);
    if (flag) { //压平
        earth.Event.OnCreateGeometry = function (pval, type) {
            earth.ShapeCreator.Clear();
            earth.LayerManager.FlattenLayerByRegion(pval, 3);
        }
        earth.ShapeCreator.CreatePolygon();
    } else { //取消
        earth.LayerManager.ClearEffect();
    }
}