...
var selectLayer = null; //所选图层
if (parent.LayerManagement) {
    selectLayer = parent.LayerManagement.selectLayer;
} else {
    selectLayer = LayerManagement.selectLayer;
}
...
STAMP.Search = function (earth) {
     ...
     * POI查询面板展开
     */
    var DpoiSearchBtnClick = function (id) {
        if ($("#keywordSearch").attr("disabled") == "disabled") {
            return;
        }
        showDialog('html/search/keywordSerResult.html', id);
    ...
    var localSearch = function (guid, keyword, spatialObj) {
        ...
        if (parent.clearSearchResult != undefined) {
            parent.clearSearchResult(layerObj); //清除上次查询的图层数据
        }
    ...
    var showVectorMsg = function (record, location) {
        if (record != null) {
            ...
            if (parent.SYSTEMPARAMS.balloonAlpha > 0) {
    ...
    var showpoiMsg = function (obj, name) {
        if (obj != null) {
            ...
            var balloonHeight = earthHeight / 2 - 50;
            var divHeight = balloonHeight - 30;
            if (parent.SYSTEMPARAMS.balloonAlpha > 0) { //气泡透明
                htmlStr = '<div style="word-break:keep-all;white-space:nowrap;overflow:auto;width:265px;height:' + divHeight + 'px;margin-top:25px;margin-bottom:25px"><table style="font-size:16px;background-color: #ffffff; color: #fffffe">';
            ...
            htmlStr = htmlStr + '<tr>';
            htmlStr = htmlStr + '<td style="word-wrap:break-word;" width="50"></td>';
    ...
    var showPOIMsg2 = function (obj, name, index) {
        if (obj != null) {
            ...
            var balloonHeight = earthHeight / 2 - 50;
            var divHeight = balloonHeight - 30;
            if (parent.SYSTEMPARAMS != undefined && parent.SYSTEMPARAMS.balloonAlpha > 0) {
                htmlStr = '<div style="word-break:keep-all;white-space:nowrap;overflow:auto;width:265px;height:' + divHeight + 'px;margin-top:25px;margin-bottom:25px"><table style="font-size:16px;background-color: #ffffff; color: #fffffe">';
                poihtmlBallon.SetRectSize(300, balloonHeight);
                poihtmlBallon.SetIsTransparence(true);
                poihtmlBallon.SetBackgroundAlpha("0xcc");
            } else {
                htmlStr = '<div style="word-break:keep-all;font-family:Microsoft Yahei;border:1px solid #ccc;white-space:nowrap;overflow:auto;width:425px;height:385px;margin-top:15px;margin-bottom:15px"><table style="font-size:16px;background-color: #fff; color: black">';
                poihtmlBallon.SetRectSize(430, 400);
            ...
            if (attr) {
                for (var i in attr) {
                    ...
                    htmlStr += '<tr>';
                    htmlStr += '<td style="word-wrap:break-word;" width="100">' + i + ':</td>';
                    htmlStr += '<td style="word-wrap:break-word;" width="200">' + attr[i] + '</td>';
                    htmlStr += '</tr>';
                }
            } else {
                htmlStr += '<tr>';
                htmlStr += '<td style="word-wrap:break-word;" width="100">名称:</td>';
                htmlStr += '<td style="word-wrap:break-word;" width="200">' + name + '</td>';
                htmlStr += '</tr>';
            }
            htmlStr += '</table>';
            htmlStr += '</div>';
    ...
    search.DpoiSearchBtnClick = DpoiSearchBtnClick; //POI查询按钮
