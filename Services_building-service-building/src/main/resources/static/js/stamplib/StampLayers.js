...
var LayerManagement = {
    ...
    getLayerTreeData: function (layer, isDialog) {
        ...
        //数据权限
        var names = null;
        var jsoncookie = getCookie("MANAGER_INFO");
        if (jsoncookie != undefined && jsoncookie != null && jsoncookie.DATAINGO != undefined && jsoncookie.DATAINGO != null) {
            var dataInfo = jsoncookie.DATAINGO;
            names = dataInfo.split(",");
        }
        //alert(names[0]);
        ...
        for (var i = 0; i < childCount; i++) {
            ...
            //数据权限
            if (names != null && names.length > 0 && $.inArray(name, names) < 0) {
