...
STAMP.Userdata = function (earthObj) {
    ...
    function updateTree(data) {
        ...
        try {
            if (getOperObject() && getOperObject().$("#userDataTree") && getOperObject().$("#userDataTree").length > 0) {
            ...
            earth.Event.OnCreateGeometry = function (pval, type) {
                if (pval.Count) {
                    ...
                    if (isSelfIntersected) {
                        ...
                        if (userdataObj.click == true || userdataObj.click == "true") {
                        } else {
                            earth.ShapeCreator.Clear();
                        }
    ...
    var setRightDisable = function (isDisable) {
        try {
            if (parent.getOperObject() && parent.getOperObject().$("#userDataTree") && parent.getOperObject().$("#userDataTree").length > 0) {
                parent.gesertOperObject().setRightDisable(isDisable)
            }
        } catch (e) {
    ...
    var _delObj = function () {
        ...
        if (selObjNum <= 0 || selObjNum == "") {
            ...
            if (r) {
                if (parent.isUserdataTree) {
                    var thisZTree = parent.getOperObject().getUserdataTree();
                }
                var selectObjs = [];
